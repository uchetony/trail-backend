...
const {User, validateUser, validateObjectId, validateBillingData} = require('./model');


// GET REQUESTS

// get all users
endpoint.get('/stakeholders/:companyId', [auth, checkPermissionTo('readAny', 'user')],  async (req, res) => {
    ...
    const users = await User.find({"billingDetails.disco.id": `${req.params.companyId}`}).select(['-password', '-billingDetails.disco'])
    res.send(users);
})

// get current user
endpoint.get('/myaccount', [auth, checkPermissionTo('readOwn', 'user')],  async (req, res) => {
...
endpoint.post('/', async (req, res) => {
    ...
    const clientData = _.pick(req.body, ['fullName', 'email', 'password', 'phoneNumber', 'address', 'role'])
    ...
    user = new User(_.pick(req.body, ['fullName', 'email', 'password', 'phoneNumber', 'address', 'role']));
    ...
    res.header('x-auth-token', token).send({token, ..._.pick(user, ['_id', 'fullName', 'email'])});
...
endpoint.post('/:userId/verify/email', async (req, res) => {
    ...
    if(error) return res.status(400).send('NOT USER. Kindly verify your email with the device used in registering');

    // check if user exists
    const user = await User.findById({ _id: req.params.userId});
    const isUser = await bcrypt.compare(user.email, req.body.email);
    if(!isUser) return res.status(400).send('Oops! seems like this is a wrong email address');

    if(user.isVerifiedEmail) return res.status(400).send('This email is already verified');
...
endpoint.post('/:userId/billing', async (req, res) => {
    ...
    if(req.params.userId !== req.body.userId) return res.status(400).send('User not found');
    const billingData = _.pick(req.body, ['userId', 'meterId', 'companyId', 'companyName'])
